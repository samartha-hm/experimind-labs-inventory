...
projectRouter.post('/', (req: Request, res: Response) => {
  try {
    const creatorUser = (req as any).user || { id: 'usr-admin-01', name: 'Administrator', role: 'admin' };
    const newProject = ProjectManagementService.createProject(req.body, creatorUser);
...
projectRouter.patch('/:id', (req: Request, res: Response) => {
  try {
    const updaterUser = (req as any).user || { id: 'usr-admin-01', name: 'Administrator', role: 'admin' };
...
projectRouter.post('/:id/log-expense', (req: Request, res: Response) => {
  try {
    const operator = (req as any).user || {
      ...
      name: req.body.loggedByUserName || 'Administrator',
      role: 'admin'
    };
...
// 10. QA 21 CFR Part 11 Electronic Signature Sign-off
