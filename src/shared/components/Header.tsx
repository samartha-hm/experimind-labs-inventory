...
export default function Header({
  ...
  // Robust touch & click-outside dismissal + Escape key handler
  useEffect(() => {
    function handlePointerDown(e: PointerEvent) {
      const target = e.target as Node;
      if (isTenantMenuOpen && tenantMenuRef.current && !tenantMenuRef.current.contains(target)) {
    ...
    if (isTenantMenuOpen || isQuickCreateOpen) {
  ...
  }, [isTenantMenuOpen, isQuickCreateOpen]);
  ...
  return (
    <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 sticky top-0 z-20 shadow-xs transition-colors pt-[env(safe-area-inset-top,0px)]">
      <div className="px-3 md:px-6 py-2.5 md:py-3 flex items-center justify-between gap-2 md:gap-4">
        {/* Mobile Menu Button + Org Selector */}
        <div className="flex items-center gap-2 md:gap-3 flex-1 max-w-2xl min-w-0">
          {/* Mobile Hamburger Button */}
          <button
            onClick={onToggleMobileMenu}
            className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 md:hidden cursor-pointer touch-target flex items-center justify-center"
          ...
          {/* Org / Tenant Selector */}
          <div className="relative shrink-0" ref={tenantMenuRef}>
            <button
              onClick={() => setIsTenantMenuOpen(!isTenantMenuOpen)}
              className="flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1.5 rounded-xl bg-slate-100/80 dark:bg-slate-800 text-slate-800 dark:text-slate-100 text-xs font-bold shrink-0 border border-slate-200/60 dark:border-slate-700 cursor-pointer hover:bg-slate-200/70 transition-colors"
              aria-expanded={isTenantMenuOpen}
              aria-haspopup="true"
            >
              <Building2 className="w-4 h-4 text-indigo-600 dark:text-indigo-400 shrink-0" />
              <span className="truncate max-w-[100px] sm:max-w-[140px] md:max-w-none">{activeTenant.name}</span>
          ...
          <button
            onClick={onOpenCommandPalette}
            className="md:hidden p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer"
            title="Search"
